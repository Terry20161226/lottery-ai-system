...
def get_latest_draw(lottery_type):
    ...
    if lottery_type == "ssq":
        ...
        if data and "records" in data and len(data["records"]) > 0:
            record = data["records"][0]  # 最新一期在第一个
            return {
        ...
        if data and "records" in data and len(data["records"]) > 0:
            ...
            # 大乐透前区可能是 front 或 red，后区可能是 back 或 blue
        ...
        for rec in data["recommendations"]:
            ...
            for num in numbers:
                if lottery_type == "ssq":
                    ...
                    recs.append({
                        "front_balls": num.get("red", []),  # 大乐透前区用 red 字段
                        "back_balls": num.get("blue", [])   # 大乐透后区用 blue 字段
                    })
...
def format_report(ssq_results, dlt_results, ssq_draw, dlt_draw):
    """格式化中奖报告"""
    report = []
    ...
    # 总计
    total = sum(r['prize_amount'] for r in ssq_results) + sum(r['prize_amount'] for r in dlt_results)
    if total > 0:
...
def main():
    ...
    # 生成报告
    report = format_report(ssq_results, dlt_results, ssq_draw, dlt_draw)
    ...
    result_data = {
        ...
        "grand_total": ssq_total + dlt_total
    }
