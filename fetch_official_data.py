...
"""
从福彩官网获取双色球历史数据
URL: https://kaijiang.zhcw.com/zhcw/html/ssq/list.html
共 172 页
"""
...
def fetch_all_pages():
    ...
    # 备份现有数据
    backup_path = Path('/root/.openclaw/workspace/lottery/data/ssq_history.json.bak')
    if storage.ssq_history_file.exists():
        shutil.copy2(storage.ssq_history_file, backup_path)
        print(f"已备份现有数据到：{backup_path}")
    
    # 清空现有数据，准备覆盖
    print("清空现有数据，准备覆盖...")
    storage.ssq_history_file.parent.mkdir(parents=True, exist_ok=True)
    with open(storage.ssq_history_file, 'w', encoding='utf-8') as f:
        json.dump({
            'records': [],
            'total_records': 0,
    ...
    for page in range(1, 173):  # 1-172 页
        if page == 1:
            url = "https://kaijiang.zhcw.com/zhcw/html/ssq/list.html"
        else:
...
def main():
    ...
    storage = LotteryStorage()
    before_count = len(storage.get_history('ssq', 2000))
    print(f"导入前：{before_count}期")
    
    imported, skipped = fetch_all_pages()
    
    after_count = len(storage.get_history('ssq', 2000))
